...
3 sequential subagents that analyze, cluster, and verify the work queue.
...
Return JSON:
{
  "verdicts": {"item_id": {"verdict": "genuine|...", "reason": "..."}},
...
Launch Agent with Analyze output:
```
Given the analysis results and .desloppify/state.json:
...
Launch Agent with Plan output:
```
Verify the triage plan against .desloppify/state.json and actual code:

1. For each cluster, check that referenced file paths exist (use Glob)
...
After all 3 stages:
1. Update work_item statuses for dismissed items (false-positive, wontfix)
2. Write clusters and queue to state.json
